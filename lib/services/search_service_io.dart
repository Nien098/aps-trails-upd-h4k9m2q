...
enum SearchResultType {
  ...
  /// street/trail index — distinct from [trail], which is one of the
  /// user's own saved trails. Carries a [SearchResult.locality] like
  /// [street] does; the user's own trails don't.
  namedTrail,
  street,
}
...
class SearchResult {
  ...
  /// "Lincoln Avenue"s in different cities) — null for the user's own
  /// [SearchResultType.trail] results, which don't need it.
...
/// Offline search across the user's own trails and a bundled street/trail-name
/// index, so the search FAB can jump the map camera to either — the same way
/// [GuideScreen._recenter] jumps to GPS position.
///
/// The street/trail index (`assets/data/streets.sqlite`, built by
...
///
/// **Web can't use this file at all** — `package:sqlite3` is `dart:ffi`-based,
/// which doesn't compile for web. `search_service_stub.dart` is the web
/// equivalent (a flat JSON export of this same data, searched in-memory) —
/// see `search_service.dart`'s conditional-export barrel doc, and
/// `tools/export_streets_web_json.py` for how the JSON stays in sync with
/// this file's data.
class SearchService {
  SearchService._();
  ...
  Future<void> addStreets(String regionId,
    ...
    try {
      ...
      try {
        for (final r in rows) {
          stmt.execute([r.name, regionId, r.lat, r.lon, r.kind]);
        }
      } finally {
        stmt.close();
      }
  ...
  Future<List<SearchResult>> search(String query, {Region? confineTo}) async {
    ...
    if (ftsQuery != null) {
      ...
      final rows = db.select(
        '''
        SELECT s.name, s.lat, s.lon, s.region_id, s.kind FROM streets s
    ...
    // they meant than an arbitrary OSM street/trail name.
